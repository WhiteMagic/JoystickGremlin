...
def _create_function(
    lib_name: str, fn_name: str, param_types: list, return_type: type
) -> Callable:
    """Creates a handle to a dll library function.

    Args:
        lib_name: name of the library to retrieve a function handle from
        fn_name: name of the function
        param_types: input parameter types
        return_type: return parameter type

    Returns:
        Callable to the specified function.
    """
...
class Key:
    ...
    def __init__(
        self, name: str, scan_code: int, is_extended: bool, virtual_code: int
    ) -> None:
        """Creates a new Key instance.

        Args:
            name: the name used to refer to this key
            scan_code: the scan code set value corresponding to this key
            is_extended: boolean indicating if the key is an extended scan code or not
            virtual_code: the virtual key code assigned to this key by windows
        """
        self._name = name
    ...
    @lookup_name.setter
    def lookup_name(self, name: str) -> None:
        if self._lookup_name is not None:
...
def _scan_code_to_virtual_code(scan_code: int, is_extended: bool) -> int:
    """Returns the virtual code corresponding to the given scan code.

    Args:
        scan_code: scan code value to translate
        is_extended: whether or not the scan code is extended

    Returns:
        virtual code corresponding to the given scan code
    """
    value = scan_code
    if is_extended:
...
def _virtual_input_to_unicode(virtual_code: int) -> str:
    """Returns the unicode character corresponding to a given virtual code.

    Args:
        virtual_code: virtual code for which to return a unicode character

    Returns:
        Unicode character corresponding to the given virtual code
    """
...
def _unicode_to_key(character: str) -> Key | None:
    """Returns a Key instance corresponding to the given character.

    Args:
        character: the character for which to generate a Key instance

    Returns:
        Key instance for the given character, or None if an error occurred
    """
    if len(character) != 1:
        return None
...
def key_from_name(name: str) -> Key:
    ...
    If no key exists with the provided name an exception is raised.

    Args:
        name: the name of the key to return

    Returns:
        Key instance corresponding to the provided name.
    """
    ...
    if key is None:
        msg = f"Invalid key name specified: {name}."
        logging.getLogger("system").warning(msg)
        raise KeyboardError(msg)
    else:
...
def key_from_code(scan_code: int, is_extended: bool) -> Key:
    ...
    If no key exists with the provided scan code an exception is raised.
    ...
    Returns:
        Key instance corresponding to the provided scan code.
    """
    global g_scan_code_to_key, g_name_to_key
    ...
    if virtual_code == 0xFF or name is None:
        msg = f"Invalid scan code specified: ({scan_code}, {is_extended})"
        logging.getLogger("system").warning(msg)
        raise KeyboardError(msg)
    else:
