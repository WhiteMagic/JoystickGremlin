...
\begin{bclogo}[
    ...
    The curve is computed using cubic splines, which differ from the
    cubic B\'{e}zier splines used by the CryEngine. Furthermore, it is
    possible to generate splines with invalid outputs and it is the
    users' responsibility to ensure that the configured spline is valid,
    i.e.\ all output values are between $-1$ and $1$.
\end{bclogo}
...
\subsection{Custom Module Management}
...
\begin{itemize}
    ...
    \item \verb+device_id+, the hardware id of the device, predetermined by the
        decorator function that was used to decorate the function
    \item \verb+system_id+, the sequence id of the device, predetermined by the
        decorator function that was used to decorate the function
...
Only the \verb+<device name>+ and \verb+<hardware id>+ parameters are
mandatory. If \verb+<mode>+ is omitted it defaults to ``global'' and
\verb+<system id>+ is only required if multiple devices with the same
device name are present. An object created in this way has three
decorators customised for the given joystick which can be used as
follows:
\begin{minted}{python}
