...
class LogicalOperators(enum.Enum):
    ...
    @staticmethod
    def to_display(instance: LogicalOperators):
        lookup = {
            LogicalOperators.Any: "Any",
            LogicalOperators.All: "All"
        }
        value = lookup.get(instance, None)
        if value is None:
            raise error.GremlinError(f"Invalid logical operator type: {str(instance)}")
        return value

    @staticmethod
    def to_string(instance: LogicalOperators):
        lookup = {
            LogicalOperators.Any: "any",
            LogicalOperators.All: "all"
        }
    ...
    def to_enum(string: str):
        lookup = {
            "any": LogicalOperators.Any,
            "all": LogicalOperators.All
        }
        value = lookup.get(string, None)
        if value is None:
            raise error.GremlinError(f"Invalid logical operator type: {str(string)}")
        return value


class ConditionOperators(enum.Enum):
    ...
    @staticmethod
    def to_display(instance: ConditionOperators):
        lookup = {
            ConditionOperators.InputState: "Input State",
            ConditionOperators.Joystick: "Joystick",
            ConditionOperators.Keyboard: "Keyboard",
            ConditionOperators.VJoy: "vJoy"
        }
        value = lookup.get(instance, None)
        if value is None:
            raise error.GremlinError(f"Invalid condition operator type: {str(instance)}")
        return value

    @staticmethod
    def to_string(instance: ConditionOperators):
        lookup = {
            ConditionOperators.InputState: "input-state",
            ConditionOperators.Joystick: "joystick",
            ConditionOperators.Keyboard: "keyboard",
    ...
    def to_enum(string: str):
        lookup = {
            "input-state": ConditionOperators.InputState,
            "joystick": ConditionOperators.Joystick,
            "keyboard": ConditionOperators.Keyboard,
            "vjoy": ConditionOperators.VJoy
        }
        value = lookup.get(string, None)
        if value is None:
            raise error.GremlinError(f"Invalid condition operator type: {str(string)}")
        return value
