...
The *Logical Device* is shown in the list of device tabs and functions like any other physical device in that actions can be assigned to it and will be executed as usual. The difference is that the user can add any number of inputs to this device and name them to their liking.

One can map all physical device inputs to logical device inputs and then have these send the events on to vJoy devices or keyboard keys. There may be some slight overhead due to the indirection of an additional event but that should be negligible.
